...
## 2023-10-27 - Fast Path for Path Resolution in Hot Loops
**Learning:** In Python hot loops (like file watching or directory scanning), unconditionally calling `Path.resolve()` is expensive because it triggers filesystem operations (`os.stat`, `os.readlink`) for every path component.

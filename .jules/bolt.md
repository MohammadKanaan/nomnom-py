...
## 2024-05-24 - Optimize path resolution
...
## 2024-05-24 - Optimize containment checks
**Learning:** In Python hot loops, avoid using exception handling for normal control flow (e.g., try: path.relative_to(root) except ValueError:).
**Action:** Use faster boolean checks like path.is_relative_to(root) to improve performance by avoiding exception overhead.
